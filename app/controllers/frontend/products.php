...
//
    ...
    // $params['user_id'] = Tygh::$app['session']['user_id'];     
    ...
    $params = $_REQUEST;
    $params['extend'] = ['description'];
    $params['item_ids'] = !empty($department_data['users_ids']) ? implode(',', $department_data['users_ids']) : -1;
...
function get_fn_ln(){
    // $name_users = db_get_array('SELECT lastname, firstname FROM ?:users RIGHT JOIN ?:departments ON  ?:departments.user_id = ?:users.user_id ');
    $id_users = db_get_array('SELECT ?:users.user_id FROM ?:users LEFT JOIN ?:departments ON ?:users.user_id = ?:departments.user_id ');
    // 'SELECT lastname, firstname FROM ?:users LEFT JOIN ?:departments ON  ?:departments.user_id = ?:users.user_id '
    // 'LEFT JOIN ?:users ON ?:users.user_id = ?:departments_links.users_id AND ?:departments_descriptions.lang_code = ?s', $lang_code
    // $id_users = db_get_array('SELECT user_id FROM ?:departments LEFT JOIN ?:users ON ?:departments.user_id = ?:departments.department_id');
